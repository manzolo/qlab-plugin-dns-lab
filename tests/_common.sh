...
SERVER_PORT="$(_get_port dns-lab-server)"
CLIENT_PORT="$(_get_port dns-lab-client)"
if [[ -z "$SERVER_PORT" || -z "$CLIENT_PORT" ]]; then echo "ERROR: Cannot find VM ports. Are dns-lab VMs running?"; exit 1; fi
