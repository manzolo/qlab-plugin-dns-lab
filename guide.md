...
```bash
dig @localhost web.lab.qlab +short
```
...
```bash
dig @localhost web.lab.qlab AAAA +short
dig @localhost www.lab.qlab CNAME +short
dig @localhost lab.qlab MX +short
dig @localhost lab.qlab TXT +short
```

### 2.4 Use nslookup

```bash
nslookup web.lab.qlab localhost
```

### 2.5 Query SRV records

```bash
dig @localhost _http._tcp.lab.qlab SRV +short
```

**Verification:** All record types resolve correctly from the server.
...
### 3.5 Verify the new record

```bash
dig @localhost test.lab.qlab +short
```
...
```bash
dig @localhost -x 10.20.30.10 +short
```
...
### 5.1 Query a local domain

```bash
dig @localhost web.lab.qlab +short
```

### 5.2 Query multiple record types

```bash
dig @localhost mail.lab.qlab +short
```

### 5.3 Query with non-recursive flag

```bash
dig @localhost web.lab.qlab +norecurse
```

Since the server is authoritative for `lab.qlab`, it answers directly without recursion.

### 5.4 Observe caching

```bash
dig @localhost web.lab.qlab | grep "Query time"
dig @localhost web.lab.qlab | grep "Query time"
```

The second query should be faster (cached).

**Verification:** Local domain resolution works correctly on the server.
...
### Record not resolving from server
```bash
dig @localhost web.lab.qlab
```
